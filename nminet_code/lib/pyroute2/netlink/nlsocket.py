        ...
        with self.lock[msg_seq]:
            ...
            while not enough:
                ...
                if msg_seq == 0 and self.backlog[0]:
                    ...
                    break
                elif msg_seq != 0 and self.backlog.get(msg_seq, None):
                    # Any other msg_seq.
                    #
                    ...
                    if self.read_lock.acquire(False):
                        ...
                        # vbursov:
                        #
                        # Thread desiring to get new data can enter here just after
                        # another thread, which was waiting for event here already (IPDB,
                        # for example), resulting in input queue empty, all events
                        # processed and put in backlog already, thus, blocking in recv()
                        # until any new event is received.
                        #
                        # Check again if backlog contains data we need.
                        #
                        # msg_seq is 0 in IPDB, and it's OK for it to block in recv()
                        #

                        if msg_seq != 0:
                            with self.backlog_lock:
                                exists = bool(self.backlog.get(msg_seq, None))
                            if exists:
                                self.read_lock.release()
                                #logging.info("event already present, skipping read")
                                continue
